...
declare -A EXCLUDE=(
  ...
  [test-bd-inline-body-guard.sh]="real-bd-only by construction — its whole claim is that a sentinel-creating substitution never fires against a real bd store, and its negative control needs a real destructive bd write; in the clean room it would SKIP that half and could look green on a vacuous run. Run it locally against real bd; the decision logic is covered fail-closed by bin/test_sable_inline_body_guard.py, which pytest already collects regardless of this file. [permanent: SABLE-qwthx SABLE-59zu]"
...
declare -A COVERS=(
  ...
  [test-role-card-install.sh]="bin/sable-orchestration-install templates/multi-manager/agents.yaml templates/multi-manager/roles/lincoln.md templates/multi-manager/roles/optimus.md templates/multi-manager/roles/tarzan.md templates/multi-manager/roles/chuck.md templates/multi-manager/roles/columbo.md templates/multi-manager/roles/rudy.md templates/multi-manager/roles/sherlock.md templates/multi-manager/roles/victor.md"
