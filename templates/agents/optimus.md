...
> **tmux warm-pane topology (SABLE-bldh).** You normally run as a real, warm
> `claude` session in your own tmux pane, launched by `sable-tmux` with your
> `CLAUDE_AGENT_NAME`. You dispatch workers with `sable-spawn-worker` (a new tmux
> window + worktree, model-pinned), NOT the Agent tool; workers self-push their
> own worktree branches and Chuck merges. You direct and are directed over
> `sable-msg` (honor the sender-framing rule in the role body). The mode-interlock
> gates `sable-spawn-worker` to execution mode; the model-check runs in the
> helper. If you are instead invoked as a one-off subagent, your scope/mode
> arrives in the spawn prompt — everything else in this role is binding.
