...
assert_grep "$ROLE" "Planning producers are subagents" "role distinguishes planning producer subagents"
assert_no_grep "$ROLE" "named subagents under your" "role drops retired all-roster-as-subagents topology"
assert_no_grep "$ROLE" "manager subagents" "role never describes execution managers as subagents"
assert_no_grep "$ROLE" "terminal: Chuck" "role drops retired separate-Chuck-terminal topology"
assert_no_grep "$ROLE" "You spawn managers" "role never tells Lincoln to Agent-spawn execution managers"
